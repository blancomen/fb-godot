...
func _fixed_process(delta):
	if last_viewport_size != get_viewport_rect().size:
		last_viewport_size = get_viewport_rect().size
		var back_size = get_node("bg").get_texture().get_size()
		set_pos(Vector2(back_size.width/2, last_viewport_size.y - back_size.height/2))
